...
In practical terms, this rate of increase translates to prohibitively slow model fitting. 
...
Both Bayesian estimation programs in this chapter use the Metropolis sampling algorithm, one of the most popular MCMC algorithms. 

Stan uses HMC sampling and a ``Metropolis reject step'' \citep{STANtheMan}. spBayes, for our purposes, uses a ``Metropolis-within-Gibbs'' sampler \citep{Finley2015}.

\subsection{Gibbs Sampler \citep{Banerjee2014}}
In high dimensional problems that are now common, the Gibbs Sampler achieves success, in part, because it iteratively reduces the dimension of the problem. To understand how, and give the algorithm, define parameter vector $\pmb{\theta} = (\theta_{1}, \dots, \theta_{k})'$, and assume we can draw samples from $p(\theta_{i}|\theta_{j \neq i},y)$ directly, or using the Metropolis algorithm described in the next section. The Gibbs sampler proceeds with the following steps, for iterations $(t \in 1:T)$. 
        \begin{itemize} 
        ...
        \end{itemize} 
With certain conditions met, as they are in MCMC usage, this sequence of draws converges to (correlated) draws from the true posterior distribution $p(\pmb{\theta}|\pmb{y})$ \citep{Banerjee2014}.

\subsubsection{Metropolis Algorithm \citep{Banerjee2014}} 
Typically the closed form for the target distribution, the posterior of interst, is unavailable. However, it will be available up to a proportionality constant, which is easy to see in the familiar Bayes proportionality, for observation vector $\pmb{y}$ and parameter vector $\theta$.
$$p(\pmb{\theta}|\pmb{y}) \propto f(\pmb{y}|\pmb{\theta})p(\pmb{\theta})$$
We see that the kernal, or the portion that contains the parameters of interest, is available; call it $h(\pmb{\theta})$. The Metropolis algorithm leverages this information to take a sequence of draws whose distributions converge to the target distribution. The draws of interest are full conditionals, $p(\theta_{i}|\pmb{\theta}_{-i})$, effectively reducing a possibly high dimensional problem to a series of lower dimension problems. Before the algorithm procedds, one must specify a candidate density, $q(\pmb{\theta}_{t}|\pmb{\theta}_{t-1})$, such that
  \begin{enumerate}
  \item Candidate density has non-zero density for all possible values of $\pmb{\theta}_{t}$ and $\pmb{\theta}_{t-1}$; 
  \item The candidate density is symmetric, $q(\pmb{\theta}_{t}|\pmb{\theta}_{t-1}) = q(\pmb{\theta}_{t-1}|\pmb{\theta}_{t})$.
  \item We can readily draw sample from $q(\pmb{\theta}_{t}|\pmb{\theta}_{t-1})$.
  \end{enumerate}

Next, choose an initial value for $\pmb{\theta}$, call it $\pmb{\theta}_{0}$. Then, the Metropolis algorithm steps follow, for $(t \in 1:T)$, where T is some prescribed number of iterations.
\begin{enumerate}
\item Draw candidate value of $\pmb{\theta}$, call it $\pmb{\theta}^{*}$, from $q(\pmb{\theta}^{*}|\pmb{\theta}_{t})$.
