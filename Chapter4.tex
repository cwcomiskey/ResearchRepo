...
\overset{D}{=} 
\Big[ \langle \psi_{k}, \pmb{W} \rangle \Big]_{l} $$

Simplify notation by letting $\pmb{C}_{l,k} = \langle \psi_{l}, \psi_{k} \rangle$, and $ \pmb{G}_{l,k} = \langle \psi_{l}, - \Delta \psi_{k} \rangle$, so that
$$ \left(
\kappa^{2} \pmb{C} + \pmb{G} \right) \pmb{\omega} \overset{D}{=} \text{N}(\pmb{0},\pmb{C}).$$
Thus, $\pmb{\omega} \sim \text{N}(\pmb{0}, \pmb{Q}_{\kappa}^{-1})$, where:
...
% \end{equation}
Note that this solution, $\pmb{\omega}$, specifies the distribution of the weights $\omega_{k}$ in the piecewise linear basis function representation of $\widetilde{\pmb{w}}(\pmb{s})$ (equation \ref{eq:basisrep}), not $\widetilde{\pmb{w}}(\pmb{s})$ itself.
