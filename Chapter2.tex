...
With this range of box sample sizes, the non-extreme resolution choices contain both boxes with exceedingly small sample sizes {\it and} boxes with unnecessarily large sample sizes. Figure 1.4 shows Peralta's data---the same data---at six resolutions. 
        \begin{figure}[H]
      	\centering
...
Figure 1.8 shows the VR heat map for every iteration, for stopping rule $n_{b} < 200$.
        \begin{figure}[H]
      	\centering
...
In this section we apply the VR algorithm to the same data, but with stopping rule $n_{b} < 100$, to show how the procedure and outcome differs. Figure 1.9 shows heat maps for all six VR iterations.
        \begin{figure}[H]
      	\centering
...
% Third, game states. The game state includes the count (number of balls and strikes), the number of outs, and baserunner presence/absence on each of the three bases. The level of each variable, sometimes in combination with one another, influences strategy. For example, the penalty for throwing a pitch outside the strike zone---a ``ball''---increases with each additional ball in an at bat; the hitter automatically reaches base (his goal) on the fourth ball. Our data includes pitches from all game states, so we omit game states from our interpretive toolbox. Instead, we focus on game theory and pitch location error rationale. 
