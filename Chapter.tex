...
This clever formulation offers relatively simple partial derivatives for the change in position and momentum over time. For $i = 1,\dots, d$:
\begin{align}
...
\begin{align}
\frac{d q_{i}}{dt} &=  [\pmb{M}^{-1}p]_{i} \\
\frac{d p_{i}}{dt} &= \frac {\partial \left[ \text{log}f_{q}(q) \right]}{\partial q_{i}}
\end{align}
The solutions to these two differential equations, that is $q(t)$ and p(t) such that (8) and (9) hold, give the rate of change in position and momentum at time t. 
