...
`Big n problem' burdens can be mitigated further, by optimizing coding techniques. We highlight some such techniques here, and include the complete .stan script in Appendix A for reference. 

\begin{itemize}
\item Informative prior on length-scale parameter for identifiability \citep{Gelman}
\item Diagonal noise; \cite{Trangucci}
\begin{verbatim}
for (n in 1:N)
...
\item 
``In general, it's hard to run complex models in Stan without specifying proper priors (rather than uniform, as you have here) for all your coefficients, especially if your coefficients are on different scales.'' \citep{Trangucci}    

I would suggest using a distribution for l with a sharper tail, maybe a log-normal or a normal. If the data isn't informative about your length-scale, you'll need to pin it down with a prior. See if you can come up with plausible upper and lower bounds on your length-scale, and then use a distribution to put soft constraints on l. Without specifying stronger priors on l, you can run into trouble where your GP can act as a second constant term in your regression for large draws of length-scale and large draws of alpha. You should also put priors on sig for this reason. 
...
\verb|y ~ multi_normal(mu, diag_matrix(sigma)); | \\
it's **much** faster to produce the same result with 
a vectorized scalar version: \\
\verb|y ~ normal(mu, sigma);| \\

And, instead of: \\
\verb|Z ~ multi_normal_cholesky(zero, I_N); // Standard MVN| \\ 
This is where you just want to do this: \\
 \verb|Z ~ normal(0, 1);| \\
That puts an independent standard normal on each component of Z.
