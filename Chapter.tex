...
\subsection{Numerical Optimization of Hamiltonian Monte Carlo in Stan} % =================
...
\subsubsection{Predictive Process Models in spBayes}




\subsection{Stochastic Partial Differential Equations and Integrated Nested Laplace Approximations} % =================
...
where $\pmb{c}$ and $\pmb{\mu}$ depend on a second order Taylor expansion of $\sum_{i} \text{log }p(y_{i}|x_{i})$, about the mode $x_{0}(\pmb{\theta}) = \text{argmax}_{x}p(\pmb{x}|\pmb{y},\pmb{\theta})$ \citep{Rue2007}. A numerical optimzation algorithm caluculates $x_{0}(\pmb{\theta})$, and various methods exist for determining the form of $\pmb{c}$ and $\pmb{\mu}$ \citep{Rue2009}. For example, one formulation gives the following mean and variance for the Gaussian approximation \citep{Lindstrom2014},
            \begin{align}
           ...
           \text{V}_{\pmb{x}_{0}}(\pmb{x}|\pmb{y,\theta}) & \approx \big( \pmb{Q} - \text{diag}(f''(\pmb{x}_{0})) \big)^{-1}, 
            \end{align}
for latent random field  $\pmb{x} \sim \text{N}(\pmb{\mu}, \pmb{Q}^{-1})$. Step 2 uses this Gaussian approximation, $p_{G}(\pmb{x}|\pmb{\theta},\pmb{y})$.
...
For a given $\pmb{\theta}$, let $\pmb{x}_{0}(\pmb{\theta}) = \text{argmax}_{x}p(\pmb{x}|\pmb{y},\pmb{\theta})$. Then,
...
where the Taylor expansion in $p_{G}(\pmb{x}_{0} | \pmb{y}, \pmb{\theta})$ is centered about $\pmb{x}_{0}$. This approximation matches \cite{Tierney1986} Laplace approximation. Numerical optimization gives the posterior mode, $\hat{\pmb{\theta}}_{\text{ML}} \approx \text{argmax}_{\theta} \tilde{p}(\pmb{\theta}|\pmb{y})$, an approximate maximum likelihood estimate. 
...
In summary, from a continuous domain the SPDE-INLA approxmation produces, with improved speed, posterior estimates for $p(\pmb{\theta}|\pmb{y})$, $p(\theta_{i}|\pmb{y})$, and $p(x_{i}|\pmb{y})$. The R package \verb|INLA| implements this procedure, with flexible SPDE specifications for domain triangulation.
