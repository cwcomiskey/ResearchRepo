...
No matter the performace of the previously fit model, we expect there to be unexplained spatial variation in the mean. The covariates are limited in scope and depth, and Tobler's First Law of Geography tells us that things close together in space tend to behave more similarly than things further apart \citep{Tobler1970}. Accordingly, we enhance the model by adding a spatially correlated random effect, to capture the unexplained spatial variation in the mean \citep{Banerjee2008}. 

\subsection{Gaussian Random Field}

A Gaussian random field is a popular, effective, and feasible distribution for spatial random effects \citep{Gelfand2010}. Let random variable $\pmb{w}(\pmb{s})$, for $\pmb{s} \in \pmb{D} \subseteq \pmb{R}^{2}$, be distributed multivariate Normal with mean $\pmb{0}$, and covariance matrix $\Sigma(\pmb{\theta})$, where $\pmb{\theta}$ are covariance parameters \citep{Haran2011}.\footnote{Where $\Sigma(\theta)$ is symmetric and positive definite.}
$$\pmb{w} | \pmb{\theta} \sim MVN(\pmb{0}, \Sigma(\pmb{\theta})) $$

Including a random effect defined in this way retools (2) as a {\it spatial} generlatized linear {\it mixed} model (SPGLMM).
...
Spatial correlation, notoriously, imposes considerable computational burdens \citep{Banerjee2008}. A Gaussian random field has an $n \times n$ correlation matrix, whose inverse must be evaluated at every MCMC iteration. This feature contributes a considerable portion of the computational cost   These burdens can be mitigated, somewhat, by coding techniques and maneuvers. We highlight some such techniques here, and include the complete .stan script in Appendix A as a reference. 
