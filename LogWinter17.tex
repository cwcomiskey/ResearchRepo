...
* {\bf Smoothness} = differentiability, \# of continuous derivatives (Wikipedia) \\

* {\bf Stochastic calculus} = ``branch of mathematics that operates on stochastic processes. It allows a consistent theory of integration to be defined for integrals of stochastic processes with respect to stochastic processes. It is used to model systems that behave randomly.
...
\subsection*{Spatial Data Analysis: Matern, Whittle, Stochastic Laplace}
\citep{Schabenberger2004}
\begin{itemize}
...
Stochastic Laplace equation (Z: Matern, $\epsilon$: white noise):
...
\citep{Lindstrom2014}
\begin{itemize}
\item Kriging, ``Big N'', GMRFs (1-dim AR1 is simplest example), precision matrix $Q = \Sigma^{-1}$, neighbours simplify conditional expectation $\rightarrow$ sparse precision matrix, brief computational details, create Q from Matern as solution to SPDE 
\item Matern produces Markov field for $\nu \in \mathbb{Z}$ for $\mathbb{R}^{2}$
...
\item {\bf Stochastic weak solution} to the SPDE:
      $$ \left[ \left< \psi_{j}, (\kappa^{2} - \Delta)^{\alpha/2} \psi_{i} x_{i} \right> \right]_{k = 1, \hdots, n} \overset{D}{=} [ \langle \psi_{j}, \mathcal{W} \rangle ]_{k = 1, \hdots, n} $$
      ...for each set of test functions $\{\psi_{k}\}$.
\item Definition:
$$ \langle f, g \rangle = \int f(\pmb{u}) g(\pmb{u}) d\pmb{u} $$ \citep{Lindgren2011}
